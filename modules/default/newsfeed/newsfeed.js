       ...
       updateDom: function () {
             if (this.newsItems.length === 0) {
              ...
              let wrapper = document.createElement("div");
               wrapper.classList.add("small", "bright", "newsfeed");

               let newsItem = this.newsItems[this.activeItem];

               let title = document.createElement("div");
               title.classList.add("bright", "medium", "light", "newsfeed-title");
               title.innerHTML = newsItem.title;

               let description = document.createElement("div");
               description.classList.add("light", "small", "newsfeed-description");
               ...
               const animationType = animationConfig.type || 'fadeIn';
               const animationDuration = animationConfig.duration || 1000;

               // 选择要应用动画的元素
               const newsElement = wrapper;

               // 添加动画效果
               animateCSS(newsElement, animationType, animationDuration);

               // 监听动画结束事件
               newsElement.addEventListener('animationend', () => {
               console.log(`${animationType} animation ended`);
               });
